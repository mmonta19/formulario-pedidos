...
const field = (name, label, value, error = '', type = 'text') => `
  <label>${label}</label>
  <input type="${type}" name="${name}" data-role="customer-field" value="${value || ''}" />
  ${error ? `<div class="error">${error}</div>` : ''}
`;


function ensureCustomerDraft() {
  if (!customerDraft) customerDraft = { ...state.customerData };
  return customerDraft;
}

function commitCustomerDraft() {
  if (!customerDraft) return;
  state.customerData = { ...customerDraft };
  persist();
}
...
function render() {
  ...
  if (state.step !== 2) customerDraft = null;
  bind();
}
...
function bind() {
  ...
  app.querySelectorAll('[data-action="type"]').forEach((el) => el.onclick = () => {
    dispatch('SET_TYPE', el.dataset.value);
    customerDraft = { ...state.customerData };
    dispatch('SET_STEP', 2);
  });
  app.querySelectorAll('input[data-role="customer-field"]').forEach((el) => {
    el.oninput = () => {
      ensureCustomerDraft();
      customerDraft = { ...customerDraft, [el.name]: el.value };
    };
    el.onblur = () => commitCustomerDraft();
  });
  app.querySelectorAll('input[data-role="cart-qty"]').forEach((el) => el.oninput = () => {
    dispatch('UPDATE_CART_QTY', { id: el.dataset.id, qty: Number(el.value) || 0 });
  });
  app.querySelectorAll('[data-action="back"]').forEach((el) => el.onclick = () => {
    if (state.step === 2) commitCustomerDraft();
    dispatch('SET_STEP', state.step - 1);
  });
  ...
  app.querySelectorAll('[data-action="save"]').forEach((el) => el.onclick = () => {
    if (state.step === 2) commitCustomerDraft();
    else persist();
    toast('Guardado');
  });
  ...
  app.querySelectorAll('[data-action="add-item"]').forEach((el) => el.onclick = () => {
    ...
    const system = state.navPath[1] || node.label;
    const trail = state.navPath.length > 2 ? state.navPath.slice(2) : [];
    const all = flattenProducts(node, cat.label, system, trail);
